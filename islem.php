...
if (isset($_POST['cariekle'])) {
  ...
  if (isset($_POST['cariduzenle'])) {
    ...
    //tanım Ekleme
    if (isset($_POST['tanimekle'])) {
      $tanimkaydet=$db->prepare("INSERT INTO tanimlar_tbl SET
        tanim_ad=:ad,
        tanim_boyut=:boyut,
        tanim_adet=:adet,
        tanim_kg=:kg,
        tanim_tur=:tur
        ");
        $insert=$tanimkaydet->execute(array(
          'ad' => $_POST['tanim_ad'],
          'boyut' => $_POST['tanim_boyut'],
          'adet' => $_POST['tanim_adet'],
          'kg' => $_POST['tanim_kg'],
          'tur' => $_POST['tanim_tur']
        ));

        if ($insert) {
          Header("Location:admin/tanimlar.php?durum=ok");
        }
        else{
          Header("Location:admin/tanimlar.php?durum=no");
        }
      }

      //tanım düzenleme
      if (isset($_POST['tanimduzenle'])) {

        $tanimsor=$db->prepare("SELECT * FROM tanimlar_tbl where id=:id");
        $tanimsor->execute(array("id" => $_POST['id']));
        $tanimcek=$tanimsor->fetch(PDO::FETCH_ASSOC);

        $tanimduzenle=$db->prepare("UPDATE tanimlar_tbl SET
          tanim_ad=:ad,
          tanim_boyut=:boyut,
          tanim_adet=:adet,
          tanim_kg=:kg,
          ...
          WHERE id={$_POST['id']}
          ");
          $update=$tanimduzenle->execute(array(
            'ad' => $_POST['tanim_ad'],
            'boyut' => $_POST['tanim_boyut'],
            'adet' => $_POST['tanim_adet'],
            'kg' => $_POST['tanim_kg'],
        ...
        if($_GET['tanimsil']=="ok") {
          ...
          $select = $db->prepare("SELECT * FROM tanimlar_tbl where id=:id");
          $select->execute(array('id' => $_GET['id']));
          $bul = $select->fetch(PDO::FETCH_ASSOC);

          $sil=$db->prepare("DELETE FROM tanimlar_tbl WHERE id=:id");
          $kontrol=$sil->execute(array('id' => $_GET['id']));
        ...
        //geneltanim Ekleme
        if (isset($_POST['geneltanimekle'])) {
          $geneltanimkaydet=$db->prepare("INSERT INTO geneltanim_tbl SET
            ...
            if ($insert) {
              Header("Location:admin/geneltanimlar.php?durum=ok");
            }
            else{
          ...
          //geneltanim düzenleme
          if (isset($_POST['geneltanimduzenle'])) {

            $geneltanimsor=$db->prepare("SELECT * FROM geneltanim_tbl where id=:id");
            $geneltanimsor->execute(array("id" => $_POST['id']));
            $geneltanimcek=$geneltanimsor->fetch(PDO::FETCH_ASSOC);

            $geneltanimduzenle=$db->prepare("UPDATE geneltanim_tbl SET
              iscilik_maaliyet=:iscilik,
              tasarim_maaliyet=:tasarim,
              baski_maaliyet=:baski,
              diger_maaliyet=:diger
            ...
            if($_GET['geneltanimsil']=="ok") {
              ...
              $select = $db->prepare("SELECT * FROM geneltanim_tbl where id=:id");
              $select->execute(array('id' => $_GET['id']));
              $bul = $select->fetch(PDO::FETCH_ASSOC);

              $sil=$db->prepare("DELETE FROM geneltanim_tbl WHERE id=:id");
              $kontrol=$sil->execute(array('id' => $_GET['id']));
